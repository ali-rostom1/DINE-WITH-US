...
<body class="bg-gray-100 text-gray-800 no-scrollbar">
  ...
  <main class="min-h-screen w-full relative" style="background-image: url('../assets/images/admin_menu_background.jpg');">
        <div class="max-w-5xl h-full mx-auto px-4 py-4 flex flex-col items-center gap-10">
            ...
            <div class="flex justify-between w-[90%] mx-auto items-center mt-10 z-10">
                <span class=" font-bold text-4xl text-blue-500 z-10">Dishes</span>
                <button id="addDish" class="px-5 py-2 bg-blue-500 text-white rounded-md hover:bg-blue-700 ">
                    Add Dish
                </button>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 z-10">
                <?php
                  ...
                  foreach($res as $el){
                    if ($el['img_URL'] != 'assets/images/placeholder.jpg') {
                      $imageBase64 = base64_encode($el['img_URL']);
                      $imageType = 'image/png';
                      $imageDataUrl = "data:$imageType;base64,$imageBase64";
                    }else {
                      $imageDataUrl = '../'.$el['img_URL'];
                    }
                    echo '<div data-modal-target="choice-modal" data-modal-toggle="choice-modal" id="'.$el["id_dish"].'" class="dish-card bg-white p-6 shadow-md rounded-lg hover:scale-110 transition duration-300">
  ...
  <script src="../assets/js/adminDish.js"></script>
</body>
</html>